...
def run_training():
    ...
    torch.manual_seed(5)
    word_embedding = rnn_lstm.word_embedding( vocab_length= len(word_to_int) , embedding_dim= 100)
    rnn_model = rnn_lstm.RNN_model(batch_sz = BATCH_SIZE,vocab_len = len(word_to_int) ,word_embedding = word_embedding ,embedding_dim= 100, lstm_hidden_dim=128)
...
def pretty_print_poem(poem):  # 令打印的结果更工整
    poem = poem.replace(start_token, '')
    poem = poem.split(end_token)[0]
    poem_sentences = [s for s in poem.split('。') if s]
    if poem_sentences:
...
def gen_poem(begin_word):
    ...
    word_embedding = rnn_lstm.word_embedding(vocab_length=len(word_int_map), embedding_dim=100)
    rnn_model = rnn_lstm.RNN_model(batch_sz=64, vocab_len=len(word_int_map), word_embedding=word_embedding,
    ...
    poem = begin_word
    poem_with_start = start_token + begin_word
    word = begin_word
    while word != end_token:
        input = np.array([word_int_map[w] for w in poem_with_start],dtype= np.int64)
